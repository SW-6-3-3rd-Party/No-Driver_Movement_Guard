...
SERIAL_USE_STTY = os.getenv("MON_SERIAL_USE_STTY", "1") == "1"
...
def apply_serial_stty_profile() -> None:
    if not SERIAL_USE_STTY:
        return
    if os.name != "posix":
        return
    ...
    try:
        ...
        if completed.returncode == 0:
            print(f"[SERIAL] stty applied: {' '.join(cmd)}")
        else:
            err = completed.stderr.strip() or completed.stdout.strip() or f"code={completed.returncode}"
            print(f"[SERIAL] stty failed: {err}")
    except FileNotFoundError:
        print("[SERIAL] stty not found; continue with pyserial defaults")
    except Exception as exc:
        print(f"[SERIAL] stty error: {exc}")
...
def health():
    ...
    return jsonify({
        "ok": True,
        "status": serial_message,
        "state": serial_message,
        "health_status": "ok",
        "connected": bool(serial_state.get("connected")),
        "is_connected": bool(serial_state.get("connected")),
        "stream_connected": bool(serial_state.get("connected")),
        "retrying": bool(serial_state.get("retrying")),
        "is_retrying": bool(serial_state.get("retrying")),
        "stream_retrying": bool(serial_state.get("retrying")),
...
=======
        "serial_use_stty": SERIAL_USE_STTY,
...
def api_serial_status():
    ...
    serial_message = "connected" if state.get("connected") else "reconnecting"
    return jsonify({
        "ok": True,
        "status": serial_message,
        "state": serial_message,
        ...
        "port": SERIAL_PORT,
        "baudrate": SERIAL_BAUDRATE,
        "use_stty": SERIAL_USE_STTY,
        "connected": bool(state.get("connected")),
        "is_connected": bool(state.get("connected")),
        "stream_connected": bool(state.get("connected")),
        "retrying": bool(state.get("retrying")),
        "is_retrying": bool(state.get("retrying")),
        "stream_retrying": bool(state.get("retrying")),
        "message": serial_message,
